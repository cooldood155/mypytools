...
def to_list(
    *entries: Any,
    ...
    if unwrap_layers < -1 or unwrap_layers == 0:
        raise ValueError("unwrap_layers must be -1 (infinite) or a positive integer >= 1.")
    ...
    def unwrap(entry: Any, depth: int) -> Iterable[Any]:
        ...
        Example::
            ...
            unwrap(RGB(255, 0, 0))      # [RGB(255, 0, 0)]
    ...
    flat_result = [
        item for entry in entries if keep(entry)
        for item in unwrap(entry, unwrap_layers) if keep(item)
    ]
